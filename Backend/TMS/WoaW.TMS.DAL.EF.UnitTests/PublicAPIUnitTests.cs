...
using Lacerta.Contracts.TMS;
using Lacerta.Entities;
...
namespace WoaW.TMS.Tasks.UnitTests
{
    ...
    public class PublicAPIUnitTests
    {
        ...
        {
            //arrange
            var manager = new Party { Id = "manager" };
            var worker = new Party { Id = "worker" };
            var im  = new NotificationCenter();
            var rm = new ResourceManager(im, new Party[] { worker }) { InManualMode = true };
            rm.Rules.Add(new FindUserForSameTaskRule(rm)); 
            var task = new WorkEffort(new WorkEffortType() { Id = "1" }, new Task1() { Id = "1" });

            //act
            var assignment = rm.AssignTask(manager, worker, task);
            ...
            Assert.AreEqual(worker, assignment.AssignedTo);
        }
        /// <summary>
        /// метод проверяет тот случай когда:
        ...
        {
            //arrange
            var role1 = new ApplicationRole() { Id = "1", Name = "Role1" };
            var manager = new Party { Id = "manager" };
            var worker = new Party(new ApplicationRole[] { role1 }) { Id = "worker" };
            var im = new NotificationCenter();
            var rm = new ResourceManager(im, new Party[] { worker }) ;
            rm.Rules.Add(new FindUserForSameTaskRule(rm));
            var task1 = new WorkEffort(new WorkEffortType() { Id = "1" }, new Task1() { Id = "1" }) { RequerdRole = role1 };
            ...
            Assert.AreEqual(0, rm.WorkEfforts.Count);
            Assert.AreEqual(1, rm.Assignments.Count);
            Assert.AreEqual(0, worker.Tasks.Count);
            Assert.AreEqual(worker, assignment1.AssignedTo);
            Assert.AreEqual(EWorkEffortStatus.Assigned, assignment1.Status);
            #endregion

            var task2 = new WorkEffort(new WorkEffortType() { Id = "2" }, new Task2() { Id = "2" }) { RequerdRole = role1 };

            //act
            var assignment2 = rm.AssignTask(manager, worker, task2);
            ...
            Assert.AreEqual(1, worker.Tasks.Count);
        ...
        public void AssignTaskAtUser_2Users_1TaskPerforms_SuccessTest()
        {
            var role1 = new ApplicationRole() { Id = "1", Name = "Role1" };
            var manager = new Party { Id = "manager" };
            var w1 = new Party(new ApplicationRole[] { role1 }) { Id = "worker1" };
            var w2 = new Party(new ApplicationRole[] { role1 }) { Id = "worker2" };
            var im = new NotificationCenter();
            var rm = new ResourceManager(im, new Party[] { w1, w2});
            rm.Rules.Add(new FindUserForSameTaskRule(rm));
            var t1 = new WorkEffort(new WorkEffortType() { Id = "1" }, new Task1() { Id = "1" }) { RequerdRole = role1 };
            ...
            var t2 = new WorkEffort(new WorkEffortType() { Id = "1" }, new Task1() { Id = "1" }) { RequerdRole = role1 };
            ...
            Assert.AreEqual(false, w1.IsBusy);
            ...
            Assert.AreEqual(EWorkEffortStatus.Canceled, assignment1.Status);
            Assert.AreEqual(0, w1.Tasks.Count);
            #endregion
        }
        /// <summary>
        ...
        public void RemoveTask_SuccessTest()
        {
            ...
            var w1 = new Party(new ApplicationRole[] { role1 }) { Id = "worker1" };
            var im = new NotificationCenter();
            var rm = new ResourceManager(im, new Party[] { w1 });
            rm.Rules.Add(new FindUserForSameTaskRule(rm));
            var t1 = new WorkEffort(new WorkEffortType() { Id = "1" }, new Task1() { Id = "1" }) { RequerdRole = role1 };
            rm.WorkEfforts.Add(t1);
            ...
            Assert.AreEqual(0, rm.WorkEfforts.Count);
            Assert.AreEqual(1, rm.Assignments.Count);
            Assert.AreEqual(EWorkEffortStatus.Canceled, a2.Status);
            Assert.AreEqual(0, w1.Tasks.Count);
            #endregion
        }
        /// <summary>
        ...
        public void AcceptTaskManually_1UsersFree_AllowManualAcceptance_SuceesTest()
        {
            ...
            var rm = new ResourceManager(im, new Party[] { w1 });
            rm.Rules.Add(new FindUserForSameTaskRule(rm));
            var t1 = new WorkEffort(new WorkEffortType() { Id = "1" , AllowManualAcceptance = true}, new Task1() { Id = "1" }) { RequerdRole = role1 };
